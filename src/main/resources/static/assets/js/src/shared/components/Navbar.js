  ...
  return pages.map(page => (
    `
      <router-link to="${page.path}">
        <a href="#" class="${className}">${page.name}</a>
      </router-link>
    `
  )).join("");
}
...
export default {
  template: `
    <nav class="relative bg-gray-900/100 after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white/10">
          ...
          <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
            ...
            <div class="hidden sm:ml-6 sm:block">
              <div class="flex space-x-4">
                ${ buildLinks("rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white") }
              </div>
            </div>
          </div>
      ...
      <el-disclosure id="mobile-menu" hidden class="block sm:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3">
          ${ buildLinks("block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-white") }
        </div>
      </el-disclosure>
    </nav>
  `
